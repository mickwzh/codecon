[**🇨🇳中文**](https://github.com/shibing624/similarities/blob/main/README.md) | [**🌐English**](https://github.com/shibing624/similarities/blob/main/README_EN.md) 
-----------------
...
![Screenshot 2024-10-27 at 8.12.01 AM.png](..%2F..%2F..%2F..%2Fvar%2Ffolders%2F0b%2Ft2kw3zj55m7908l3zg5hs1xm0000gn%2FT%2FTemporaryItems%2FNSIRD_screencaptureui_srCIMO%2FScreenshot%202024-10-27%20at%208.12.01%20AM.png)
(Dell, 2024)
...
**第二，在使用有监督学习方法之前，可以借助词向量（Embeddings，点击此处了解啥是词向量）辅助启发、扩充标签**
- _标签启发_：使用词向量/补充链接 对文本进行聚类，启发标签
...
(个人非常喜欢 Prof. Melissa Dell 的工作，严谨、实用、有深度, 另分享Prof. Melissa Dell 最新关于此篇深度学习如何应用在经济学研究中的的文章笔记） /补充链接！！！  
...
```
### 如果不清楚如何pip，请阅读[python应急基础](#python应急基础)

## 使用
### 数据准备
为尽可能减少使用者的工作量，codecon实践文本分类任务任务全流程需要且仅需要两个简单表格，请严格按照以下指南准备数据

- 在你喜欢的路径下新建一个文件夹，之后所有的结果与过程文件都将自动保存到该文件夹下
- 在该文件夹中准备好原表格(raw_file)与预测表格(pred_file)，支持.csv,.xls,.xlsx,接下来以 .xls 格式为例

- raw_file 为已经手工标记好的数据，包含 text 列和 label 列。text 列为文本，label列为文本对应的标签（从0开始的连续整数）。
  - ⚠️ label必须为从0开始的连续整数 (0,1,2,...)。必须从0开始，必须为整数，必须连续。  
  - ⚠️ 列名必须为text 和 label，不能为其他列名
...
```
#### STEP3: 在pred_file的文件夹下生成结果文件  
 - labeled_data_pred_BERTopic.csv / labeled_data_pred_LDA.csv: 对pred_file中的每条文本贴类别标签 
 - topics_description_BERTopic.csv / topics_description_LDA.csv: 每一个类别的信息（类别的代表关键词，代表文本）

-----------------

### 使用2 cl_nlp_findtrain:基于文本相似度的训练样本扩展
#### STEP1: 新建一个文件夹，准备好将要被贴标签的预测表格(pred_file)和已经贴有标签的原表格raw_file
- 支持raw_file中有多个类别，最终会分别输出从pred_file.xls中找到的每个类别的高相似度文本
- 假设类别X中有多条文本，此时pred_file.xls中某一条文本与类别X的相似度 = 与类别X中每一条文本相似度的平均值
  - ⚠️ label必须为从0开始的连续整数 (0,1,2,...)。必须从0开始，必须为整数，必须连续。  
  - ⚠️ 列名必须为text 和 label，不能为其他列名

#### STEP2: 运行代码，其中需要输入四个参数
  - data_pred: 输入pred_file的文件路径
  - data_raw: 输入raw_file的文件路径
  - language: 输入所需处理文本的语言, 中文输入 'chn', 英文输入 'eng'
  - method: 输入计算相似度（生成文本向量）时基于的模型
    - tfidf: 基于TF-IDF算法计算的文本向量，文本数量较少时不建议使用
    - word2vec: 基于word2vec算法计算的文本向量，文本数量较少时不建议使用
    - cosent: 基于bert改进算法计算的文本向量（推荐），适用于绝大多数场景
...
#### STEP3: 在pred_file的文件夹下生成结果文件
- label_{1,2...}_Extended_Results.csv 每个文件对应raw_file中每一个类别的扩展样本
  - ⚠️ 该步骤仅能辅助人工拓展样本，再生成扩张样本后，注意人工甄选后再加入训练集
  - ⚠️ 虽然cosent模型通常有更好的效果，但当文本类别的划分主要依靠其中的某些关键词划分时，tfidf, word2vec可能有更出色的效果

-----------------

### 使用3 cl_nlp_train,cl_nlp_pred基于BERT的模型训练与预测
🌟从已标记好的raw_file中选择20%的数据作为测试集合，用剩余80%的样本作为训练集合在预训练模型BERT上微调(fine tune)  
🌟将微调好的模型在pred_file做预测
#### STEP1: 新建一个文件夹，准备好将要被贴标签的预测表格(pred_file)和已经贴有标签的原表格raw_file
⚠️ label必须为从0开始的连续整数 (0,1,2,...)。必须从0开始，必须为整数，必须连续。  
⚠️ 列名必须为text 和 label，不能为其他列名
...
- batch_size: 每次喂给机器学习的学习样本，理论上该数量越多训练越平滑。但是其取决于硬件条件，codecon会根据您的电脑配置自动选择一个合适的值(通常为6~12)。如果模型训练当中提示内存不足，需要手动调低此值。
...
#### STEP3: 查看模型训练结果
- 模型训练结束后自动在data_raw所在的文件夹下保存以下几个文件
  - train_confusion_matrix.png：函混淆矩阵
  - train_model_performance.txt: 召回率/准确率/f1分数
  - train_test_label: 在20%测试集上的原标签与预测标签，帮助发现哪一类文本模型难以区分
  - model: 训练好的模型参数，之后的步骤直接调用，不需要进行任何操作

#### STEP4 使用训练好的模型在剩余样本上做预测
- benchmark: 分类的本质是模型在不同的类别上预测概率，输出概率最大的那一类。但在经济学研究的文本分析任务中通常无法预设所有类别。 例如，假设一个二分类问题，模型给定文本X 属于type1 49%的概率，type2 51%的概率，判定X属于type2。但此时 type1 不应该归位type1，与type2的任何一类。  
  - benchmark = 0 表示对所有样本进行强制分类；benchmark = 80 意味着只有当模型认为一个样本的属于某一类别的概率大与80%时才判定其属于该类别（模型的置信度为80%）
  - 如果模型在所有类别上给定的概率都小于benchmark，将该样本标记为 -1 类 （labels = -1)
...
codecon.cl_nlp_pred(data_pred = '替换为你的pred_file文件路径',
                    ...
                    batch_size=None) #可不填写。如有需求填写正整数。
```
- 在pred_file的同一文件夹下生成 pred_results.csv

-----------------

### 使用4 gai_nlp:批量调取生成式AI接口
🌟本研究使用Kimi大模型作为接口，使用前需在kimi平台申请密钥（很简单，步骤如下），codecon不收取任何中介费用  
#### STEP1 申请kimi密钥
...
### 在pred_file上直接调用大模型
- pred_file.xls 为之后需要在上面贴标签的数据，包含 text 列
- model 根据输入输出文本长度的需要，选择moonshot-v1-8k, moonshot-v1-32k, moonshot-v1-128k三种模型
- task: 输入你对任务的要求和描述，之后会对pred_file中的每一行文本执行该任务
...
- kimi的标记结果 label_gai_Results.csv 将输出到与pred_file相同的文件夹下，除了运行后全部保存之外，运行中间每10条保存一次
...
## python应急基础
- 如果你是python小白却深陷RA的ddl，或者需要立即使用codecon搬砖，或许以下几步可以帮到你
- 如果是需要用cl_nlp_train,cl_nlp_pred训练大模型，强烈推荐直接租借服务器
### 本地运行
...
- 在 我租用的实例中 点击打开工作区，点击左上角蓝色按钮，新建Jupyter Notebook
